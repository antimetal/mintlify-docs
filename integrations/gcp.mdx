...
A billing account administrator must grant the following roles to your existing Antimetal service account on each billing account included in the assessment.
...
<Step title="Create the custom role">
  ...
  | `billing.billingAccountSkus.list` | List account-visible SKUs and their metadata. |
  | `billing.billingAccountPrices.list` | List current account-specific SKU prices in bulk. |
...
<Step title="Assign the roles">
  ...
  | --- | --- | --- |
  | **Antimetal Cost Viewer** (created above) | Read supported commitment orders, discover SKUs, and retrieve customer-specific prices. | Relevant billing account. |
